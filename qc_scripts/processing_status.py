...
Stages checked:
  - raw_voltage metrics   (qc/raw_voltage/metrics/per_window/sub-XXX_<type>.csv, one row per subject)
  - raw_voltage mask      (qc/raw_voltage/masks/<label>/sub-XXX.csv)
...
Usage:
  python -m qc_scripts.processing_status \
...
def build_subject_status(registry, raw_voltage_root, bipolar_root, mask_label):
    ...
    for subject_id in subjects:
        ...
        for artifact_type in config.ARTIFACT_TYPES:
            p = config.metrics_per_window_dir(raw_voltage_root) / f'{subject_id}_{artifact_type}.csv'
            row[f'raw_voltage_metric_{artifact_type}'] = p.exists()

        mask_p = Path(raw_voltage_root) / 'masks' / mask_label / f'{subject_id}.csv'
        row[f'raw_voltage_mask_{mask_label}'] = mask_p.exists()
...
def main():
    ...
    ap.add_argument('--mask-label', default='baseline',
                     help='Which raw_voltage mask label to check for (default: baseline)')
